...
My reason for constraining the patch as much as possible is so that I can hopefully recreate the patch by hand using embroidery thread. (Update: I started this, but greatly underestimated the time & material commitments, added a WIP photo to the repo). Since it would be basically impossible to match all of the colors generated in a non-constrained patch, black & white / greyscale would make the task more do-able.
